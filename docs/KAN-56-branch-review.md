...
| H3  | Tests         | **Route catch→504 branch (route.ts:107-111) has zero coverage.** The timeout-vs-crash distinction is only reachable when `runJobs` _rejects_ while the signal is aborted; both 504 tests exit via the post-run `signal.aborted` path instead. A regression mapping timeouts to 500 would ship.                                                                                                                                                                 | route.ts:107-111 vs cron-harness.test.ts:504-525,527-576 | One test: rejecting `runJobs` under an aborted signal → 504.                                                                                                      |

...
for *why* things look the way they do — the tokens, the rules, and the
...
The difference between the two is *density*, never identity. Same fonts
...
Status is a closed vocabulary of tinted chips. These are *not* the brand
...
- **Serif (Noto Serif)** is for *headlines only*: the marketing hero, section
...
- No filled/duotone/outline mixing. A filled icon is permitted *only* to show
...
| In-mockup primary | `bg-brand-deep` (teal), white text — *only inside product mockups* |
...
mockups and the app use the *same* component after KAN-86.
